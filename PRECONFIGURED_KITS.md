...
### danjuliodesigns, LLC gCore

Choose the predefined board danjuliodesigns gCore in `Component config->LVGL TFT Display controller` to properly configure the ILI9488 driver for operation with gCore.

Make other changes with menuconfig as follows.

1. Select 240 MHz ESP32 clock and Rev 3 chip type in `Component config->ESP32-specific`
2. Select Swap 2 bytes of RGB565 color in `Component config->LVGL configuration`
3. Select Landscape Inverted display orientation in `Component config->LVGL TFT Display controller`
4. Select FT6X06 touch panel controller in `Component config->LVLG Touch controller`
5. Configure the touch panel controller to Swap X and Y coordinate and Invert X coordinate values in `Component config->LVGL Touch controller->Touchpanel Configuration`
